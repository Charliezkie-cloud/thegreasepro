...
export default function BookingForm() {
  ...
  async function bookingFormOnSubmit(e: SubmitEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const jsonRes: { message?: string; error?: string; } = await res.json();
      const messageRes = jsonRes.message;
      const errorRes = jsonRes.error;

      if (res.ok && messageRes) {
        populateBookingSuccess(messageRes);
        return setBookingModal(true);
      }

      if (messageRes)
        populateBookingError(messageRes);
      if (errorRes)
        populateBookingError(errorRes);
  ...
  // Helpers
  function populateBookingError(message: string) {
    setBookingSuccess("");
    setBookingError(message);
  }

  function populateBookingSuccess(message: string) {
    setBookingError("");
  ...
  return (
    <>
        ...
        <div className="max-w-4xl mx-auto px-6">
          <div className="industrial-border bg-surface-container p-8 md:p-12 relative overflow-hidden">
            ...
            <form className="space-y-12" onSubmit={bookingFormOnSubmit}>
              {bookingError && (
                <div className='shadow p-4 bg-red-900/15 rounded-xl'>
                  <p className='text-red-500 text-center font-label'>{bookingError}</p>
                </div>
              )}
              ...
              <div className="space-y-8">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input value={bookingBody.firstName} onChange={(e) => setBookingBody(prev => ({ ...prev, firstName: e.target.value }))} type="text" className="input-field" placeholder="Wrench" />
                  </div>
                  <div className="space-y-2">
                    <label className="font-label text-[10px] text-on-surface-variant uppercase tracking-widest font-bold">
                      Last Name{" "}
                    ...
                    <input value={bookingBody.lastName} onChange={(e) => setBookingBody(prev => ({ ...prev, lastName: e.target.value }))} type="text" className="input-field" placeholder="Turner" />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <div className="flex">
                      ...
                      <input value={bookingBody.mobileNumber} onChange={(e) => setBookingBody(prev => ({ ...prev, mobileNumber: e.target.value }))} type="tel" className="input-field" placeholder="(555) 000-0000" />
                    </div>
                  </div>
                  <div className="space-y-2">
                    <label className="font-label text-[10px] text-on-surface-variant uppercase tracking-widest font-bold">
                    ...
                    <input value={bookingBody.emailAddress} onChange={(e) => setBookingBody(prev => ({ ...prev, emailAddress: e.target.value }))} type="email" className="input-field" placeholder="owner@garage.com" />
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <input value={bookingBody.servicesAddress} onChange={(e) => setBookingBody(prev => ({ ...prev, servicesAddress: e.target.value }))} type="text" className="input-field pl-10" placeholder="Street, City, Zip Code" />
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      ...
                      <div className="relative">
                        ...
                        <input value={bookingBody.preferredDate} min={new Date().toISOString().slice(0, 10)} onChange={(e) => setBookingBody(prev => ({ ...prev, preferredDate: e.target.value }))} type="date" className="input-field pr-10 appearance-none" />
                      </div>
                    </div>
                    <div className="space-y-2">
                      <label className="font-label text-[10px] text-on-surface-variant uppercase tracking-widest">
                      ...
                      <div className="relative">
                        ...
                        <input value={bookingBody.preferredTime} onChange={(e) => setBookingBody(prev => ({ ...prev, preferredTime: e.target.value }))} type="time" className="input-field pr-10 appearance-none" />
                      </div>
                    </div>
                  </div>
                </div>
      ...
      {/* Booking modal */}
      <Modal Title='Success' visible={bookingModal} onClose={() => setBookingModal(false)}>
        <p className='text-green-500 text-center font-label'>{bookingSuccess}</p>
      </Modal>
    </>
  );
}
